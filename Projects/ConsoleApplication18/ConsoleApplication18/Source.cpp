...
int loadresult(MODEL *model, PARTICLE Ar[], PARTICLE DLC[], D_PARAM *disp)
{
	...
	while (fgets(buf, 1000, fp) != NULL)
	{
		...
		{
		case 'S':
			sscanf(buf + 6, "%d %d", &step, &particle);
		//	printf("%d %d\n", step, particle);

			posx = (double *)malloc(sizeof(double) * particle);
			posy = (double *)malloc(sizeof(double) * particle);
			posz = (double *)malloc(sizeof(double) * particle);
			posr = (double *)malloc(sizeof(double) * particle);
			posa = (int *)malloc(sizeof(double) * particle);
			posb = (int *)malloc(sizeof(double) * particle);
			break;
		...
		case'e':
			for (int i = 0; i < particle; i++)
			{
				...
				circle(posy[i] * 500, 300 - (posz[i] * 400),
					posx[i] * 500, (posr[i]));
...
int RenderBasis(MODEL *model){
	...
	{
		glcube(0, 500, 0, 100, -deta.wallthickness, deta.wallthickness);	
	...
	{
		glcube(-deta.wallthickness, deta.wallthickness, 0, 100, 0, 500);	
